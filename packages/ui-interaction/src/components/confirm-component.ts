...
const renderCloseButton = (props: ConfirmComponent) =>
  props.options.showCloseButton
    ? html` <mwc-icon-button @click="${props._close}" id="close-btn">${interactionIcons.close}</mwc-icon-button> `
    : nothing;
  ...
  !props.options.withoutActions
    ? html`
        <div id="actions" part="actions">
          <mwc-button @click="${props._cancel}" id="cancel-btn">${props.options.cancelLabel}</mwc-button>
          <mwc-button @click="${props._accept}" id="accept-btn" autofocus>${props.options.acceptLabel}</mwc-button>
        </div>
      `
    : nothing;
...
const renderDialog = (props: ConfirmComponent) => html`
  <paper-dialog id="dialog" type="${props.options.type}" ?fullScreen="${props.options.fullScreen}" ?modal="${props.options.modal}" part="dialog">
    ...
    <div id="content" part="content">
      <paper-dialog-scrollable class="${props.options.classifiers}"> ${renderContent(props.options, props)} </paper-dialog-scrollable>
      ${renderActions(props)}
    </div>
  </paper-dialog>
`;
