...
export class UxlVirtualizer extends propertiesObserver(LitElement) {
  @property()
  public itemsToDisplay = 25;

  @property()
  public itemsToDisplayRange = 5;

  @property()
  public items: any[];

  @property()
  public runWayPercent = 15;
  ...
  @property()
  private topRunway = 200;

  @property()
  private bottomRunway = 200;
  ...
  async scrollDownAppendDOM() {
    ...
    if (
      this.container.scrollHeight -
        ...
        this.bottomRunway &&
      this.canAppendOrPrependItem()
    ) {
      const elementToAppend = this.getNextElementToAppend();
      if (elementToAppend) {
  ...
  async scrollDownDropDOM() {
    if (
      this.container.scrollTop > this.topRunway &&
      this.canDropFirstOrLastItem()
    ) {
      this.dropItem();
  ...
  async scrollUpPrependDOM() {
    if (
      ...
      const elementToPrepend = this.getNextElementToPrepend();
      if (elementToPrepend) {
  ...
  getNextElementToAppend() {
    const item = this.items[this.endIndex + 1];
    if (item) {
      return item;
    }
  }
  ...
  getNextElementToPrepend() {
    const item = this.items[this.firstIndex - 1];
    if (item) {
      return item;
    }
  }
  ...
  getDifferenceScroll() {
    const differenceScroll = this.lastScroll - this.container.scrollTop;
  ...
  canAppendOrPrependItem() {
    return (
      this.displayedItems.length <=
        this.itemsToDisplay + this.itemsToDisplayRange ||
      this.displayedItems.length >= this.itemsToDisplay
    );
  }

  getRunways() {
    const value = (this?.container?.scrollHeight * this.runWayPercent) / 100;
    const result = Math.round(value);
    return result;
  }

  setRunways() {
    const runWay = this.getRunways();
    if (runWay) {
